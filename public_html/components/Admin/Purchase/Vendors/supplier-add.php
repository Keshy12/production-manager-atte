...
try {
    ...
    $id    = $repo->create($vendorId, $name, $jobTitle, $phone, $email, $comment);
    echo json_encode(['success' => true, 'id' => $id, 'message' => 'Osoba kontaktowa dodana pomyślnie']);
} catch (\InvalidArgumentException $e) {
    echo json_encode(['success' => false, 'error' => $e->getMessage()]);
} catch (\Throwable $e) {
    error_log('supplier-add error: ' . $e->getMessage());
    echo json_encode(['success' => false, 'error' => 'Wystąpił błąd podczas dodawania osoby kontaktowej']);
}
exit;
