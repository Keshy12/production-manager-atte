...
$deviceTypeFilter = $_POST["device_type_filter"] ?? null; // For "all" mode - specific device type
...
if ($deviceType === 'all') {
    // For "all" device types - if device_id specified, filter to specific device
    if ($deviceId && $deviceTypeFilter) {
        // Load more entries for a specific device within a group
        $type = $deviceTypeFilter;

        // Count total entries for this specific device
        $countQuery = "
        ...
        // Fetch paginated entries for this specific device
        $entriesQuery = "
            SELECT
                i.id,
    ...
    // Single device type
    $deviceIdCondition = $deviceId ? "AND i.{$deviceType}_id = $deviceId" : "";
